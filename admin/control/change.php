...
if ($num == 1) {
	$nom = mysql_num_rows(mysql_query("SELECT * FROM admin WHERE email='" . $_GET['email'] . "'"));
	if ($nom == 1) {
	...
	if ($num == 1) {
		$nom = mysql_num_rows(mysql_query("SELECT * FROM admin WHERE email='" . $_POST['email'] . "'"));
		if ($nom == 1) {
			if (mysql_query("DELETE FROM forgot WHERE email='" . $_POST['email'] . "' AND activate='" . $_POST['activate'] . "'")) {
				

				$user_q= mysql_query("UPDATE admin SET password='" .md5( $_POST['pass']) . "' WHERE email='" . $_POST['email'] . "'");
				if ($user_q){
    ...
    <div id="alert"></div>
			...
			<hr class="colorgraph">
				<h2>Success!!</h2>
				<p>You have successfully changed your password. Please <a href="../form.php">Login</a></p>
				<hr class="colorgraph">
				</div>
			</fieldset>
	</div>
...
<html>
 ...
 ?>
		...
		<script src="../assets/js/bootsrtap.min.js"></script>
